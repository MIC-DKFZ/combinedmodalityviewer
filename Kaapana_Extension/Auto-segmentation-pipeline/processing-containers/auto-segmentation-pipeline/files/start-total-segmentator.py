...
if __name__ == "__main__":
    ...
    body_part = getenv("BODY_PART", "None")
    body_part = body_part if body_part.lower() != "none" else None
    ...
    # Backward compatible logic for BODY_PART
    if body_part and task != "total" and task != body_part:
        logger.warning(f"# task: {task} disabled (selected: {body_part}) -> skipping")
        exit(126)
    ...
    # Dynamic Body Part Filtering Logic
    # If the requested task is not a standard TotalSegmentator task, check if it's a subset of 'total'
    if task not in tasks_available:
        logger.info(f"Task '{task}' not in standard tasks. Checking if it is a subset of 'total'...")
        
        # Define composite parts that map to multiple TotalSegmentator labels
        COMPOSITE_PARTS = {
            ...
            "adrenal_gland": ["adrenal_gland_right", "adrenal_gland_left"],
            "vertebrae": [
        ...
        subset_labels = []
        if task in COMPOSITE_PARTS:
            subset_labels = COMPOSITE_PARTS[task]
            logger.info(f"Found composite part '{task}' mapping to: {subset_labels}")
        else:
            # Check if it's a single label in 'total'
            total_labels = [item['label_name'] for item in seg_info_lookup_dict['total']['seg_info']]
            if task in total_labels:
                subset_labels = [task]
                logger.info(f"Found single label '{task}' in 'total' task.")
        
        if subset_labels:
            logger.info(f"Switching task to 'total' and setting roi_subset to {subset_labels}")
            task = "total"
            roi_subset = subset_labels
            
            # Filter seg_info_dict to only include the subset labels
            # We need to do this AFTER loading seg_info_dict for 'total'
        else:
            # If still not found, assert will fail below
            pass

    assert task in seg_info_lookup_dict
    seg_info_dict = seg_info_lookup_dict[task]

    # Apply filtering to seg_info_dict if roi_subset is set (and we are in 'total' task or similar)
    if roi_subset:
        logger.info(f"Filtering output seg_info.json based on roi_subset: {roi_subset}")
        original_count = len(seg_info_dict["seg_info"])
        seg_info_dict["seg_info"] = [
            item for item in seg_info_dict["seg_info"] 
            if item["label_name"] in roi_subset
        ]
        filtered_count = len(seg_info_dict["seg_info"])
        logger.info(f"Filtered seg_info entries from {original_count} to {filtered_count}")
